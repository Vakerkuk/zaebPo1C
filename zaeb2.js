...
(function() {
    ...
    // НОВАЯ КОМАНДА: gaa - показывает все варианты ответов с указанием правильного
    window.gaa = function(query) {
        const q = myQuestions;
        if (typeof query === 'number') {
            const index = query - 1;
            if (index >= 0 && index < q.length) {
                const question = q[index];
                console.log(`\n Вопрос ${query}: ${question.text}`);
                console.log('Варианты ответов:');
                question.options.forEach((option, i) => {
                    const marker = (i === question.correct) ? ' ✓ ПРАВИЛЬНЫЙ' : '';
                    console.log(`  ${i+1}. ${option}${marker}`);
                });
            } else {
                console.log(`Вопрос с номером ${query} не найден. Всего вопросов: ${q.length}`);
            }
            ...
            if (found.length === 0) {
                ...
                found.forEach(item => {
                    ...
                    item.options.forEach((option, i) => {
                        const marker = (i === item.correct) ? ' ✓ ПРАВИЛЬНЫЙ' : '';
                        console.log(`  ${i+1}. ${option}${marker}`);
                    });
                });
            }
            ...
            console.log('Использование: gaa(номер) или gaa("текст для поиска")');
        }
    };

    console.log('ga загружена. Примеры:\n  ga(1) — показать ответ на первый вопрос\n  ga("документ") — найти вопросы со словом "документ"');
    console.log('gaa загружена. Примеры:\n  gaa(1) — показать все варианты ответов для первого вопроса\n  gaa("документ") — найти вопросы и показать все варианты');
})();